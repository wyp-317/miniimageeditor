...
## 整体设计
- 模块划分：主页、相册、编辑器、导出、拍照、数据层（Room 历史记录）、拼图（选择/预览/保存）。
- 技术栈：Kotlin + Jetpack（ViewBinding、Lifecycle、协程）、MediaStore、OpenGL ES 2.0、CameraX、Media3 ExoPlayer、Coil、Room。
- 交互主线：选图/拍照 → 编辑（缩放、平移、裁剪） → 滤镜/调色 → 导出到相册；拼图主线：拼图入口 → 选择图片与方式（横/竖/四宫/九宫） → 开始拼图 → 预览保存。

## 开发遇到的困难
- 构建与网络：Gradle/AGP 版本要求高，国内仓库与 SDK 下载不稳定。
- 资源与图标：自适应图标前景缺失、占位资源编译错误、PNG 规范问题导致 AAPT2 报错。
- 编辑器交互：缩放/平移手感不佳、裁剪比例约束与越界控制、坐标反映射到原图切图。
- 滤镜与调色：预览与导出一致性、UI 文本可读性与按钮尺寸统一。
- 相机预览：返回编辑器后复到拍照页出现黑屏、权限处理时序问题。
- 媒体缩略图：视频封面未显示，需要统一加载首帧。
- 签名与产物：签名文件创建失败、路径与权限、生成 APK/AAB 的规范化交付。
- 拼图：横/竖拼尺寸统一与边缘对齐；宫格中心裁剪与铺满；系统照片选择器 UI（Photos/Albums）不可定制。
...
- 滤镜与调色：
  - 预览：在片段着色器中提供 `uFilterMode/uBrightness/uContrast/uExposure` Uniform 实时处理。
  - 导出：`BitmapUtils.applyEffects()` 在 CPU 端按相同参数与近似公式处理裁剪后的位图，保证结果与预览一致。
  - UI：统一白底黑字样式，按钮启用自适应文本与两行显示，面板白底圆角以保证可读性（参见 `docs/UI_STYLE.md`）。
...
- 拼图：选择页使用 `MaterialButtonToggleGroup` 管理模式单选；“导入画布”使用系统照片选择器（仅图片）并在返回后弹出缩放预览二次确认；横/竖拼统一到 2048 基准并先绘制背景；宫格按 2×2、3×3 中心裁剪并缩放到 2048×2048。

## 测试结论（拼图）
- 选择页：未选方式隐藏“开始拼图”；选图与方式后显示；导入画布确认后在预览页可见背景。
- 横/竖拼：多图排列正确，导出清晰；背景位图绘制优先。
- 宫格：2×2 与 3×3 铺满，中心裁剪避免拉伸；导出一致。
- 返回栏：与相册一致，仅返回箭头无标题；返回顺序正确。
